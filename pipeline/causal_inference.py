...
class CausalInferencePipeline(torch.nn.Module):
    ...
    def inference(
        ...
        # Optional: enable the streaming long-video attention mode. This stores
        # un-roped K/V and re-applies RoPE with relative indices each step so the
        # temporal position never leaves the trained range, while the KV cache
        # keeps at most `kv_cache_max_frames` latents FIFO (always retaining the
        # first `kv_cache_sink` frame(s) as an attention sink). Default off keeps
        # short-video inference byte-for-byte identical.
        self._configure_streaming(
            long_video,
            kv_cache_max_frames,
            kv_cache_sink,
            kv_cache_train_frames,
            kv_cache_position_mode,
        )
        ...
        if self.kv_cache1 is None:
            ...
            for block_index in range(len(self.kv_cache1)):
                ...
                self.kv_cache1[block_index]["local_end_index"] = torch.tensor(
                    [0], dtype=torch.long, device=noise.device)
                # drop streaming bookkeeping so each prompt starts a fresh window
                self.kv_cache1[block_index].pop("stream_abs", None)
    ...
    def _configure_streaming(
        ...
        """Toggle the streaming long-video attention mode on every self-attention
        block. Re-initialises the KV cache when the mode or window size changes so
        the cache is sized to hold `kv_cache_max_frames` latent frames.
        `kv_cache_train_frames` is the trained temporal range used as the RoPE
        position ceiling for the legacy top-aligned policy."""
        assert kv_cache_position_mode in ("top_aligned", "contiguous"), (
            f"unknown kv_cache_position_mode: {kv_cache_position_mode}")
        if long_video:
            if kv_cache_position_mode == "top_aligned":
                # The recent window is pinned to the top of the trained range; if
                # the cache could hold more frames than that range, window
                # positions would collide with the sink (or go negative).
                assert kv_cache_max_frames <= kv_cache_train_frames, (
                    f"kv_cache_max_frames ({kv_cache_max_frames}) must be <= "
                    f"kv_cache_train_frames ({kv_cache_train_frames})")
            else:
                assert kv_cache_max_frames > 0, "kv_cache_max_frames must be positive"
        desired_frames = kv_cache_max_frames if long_video else None
        if getattr(self, "_stream_cache_frames", "unset") != desired_frames:
            # mode/size changed → force a fresh cache allocation
            self.kv_cache1 = None
        self._stream_cache_frames = desired_frames

        for block in self.generator.model.blocks:
            sa = block.self_attn
            sa.kv_rope_relative = long_video
            sa.kv_cache_max_frames = kv_cache_max_frames
            sa.kv_cache_sink = kv_cache_sink
            sa.kv_cache_train_frames = kv_cache_train_frames
            sa.kv_cache_position_mode = kv_cache_position_mode
    ...
    def _initialize_kv_cache(self, batch_size, dtype, device):
        ...
        """
        kv_cache1 = []
        if getattr(self, "_stream_cache_frames", None) is not None:
            # Streaming long-video mode: cache exactly the rolling window.
            kv_cache_size = self._stream_cache_frames * self.frame_seq_length
